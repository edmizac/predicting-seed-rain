...
db_sd <- db_sd_raw %>% 
  group_by(group, random_seed, disp_day) %>% # each random seed = 1 run (~ run number)
  mutate(
    SDD = mean(SDD_seeds, na.rm = TRUE),
    SDD_sd = sd(SDD_seeds, na.rm = TRUE),
    n = n()
...
sdd_summary <- db_sd %>% 
  group_by(fragment, month, source, disp_day) %>% 
  summarise(
    SDD_mean = mean(SDD, na.rm=TRUE),
    SDD_sd = sd(SDD, na.rm=TRUE),
    n = n()
    )
sdd_summary

sdd_summary %>% dplyr::filter(source == "simulated")
...
# Save plot
ggsave(paste0(path, "/", '02_simple_SDD_disp_day_violin_nosource.png'), height = 5, width = 7)
...
# # Save plot
ggsave(paste0(path, "/", '02_simple_SDD_disp_day_grid-violin_nosource.png'), height = 5, width = 7)
